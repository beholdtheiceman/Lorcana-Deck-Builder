// src/lib/deckTextParse.js
// Canonical, client-safe home for the permissive pasted-deck-list parser.
// Lives under src/ (not api/) so browser code can import it without Vite's dev
// server proxying the request to the /api backend. The server copy at
// api/_lib/deckTextParse.js re-exports from here, keeping a single source of
// truth for both the serverless functions and the client tools.
//
// Parses "4 Card Name" / "4x Card Name" (one card per line — the common
// Dreamborn/inktable export shape) into pure {name, count} pairs; resolving
// names against the card oracle is the caller's job. Count-less lines are
// passed through flagged `implicit` so the resolver can drop them (section
// headers) unless they happen to be a real card name.
