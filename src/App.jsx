...
/**
 * Lorcana Deck Builder – Dreamborn-style two-pane (build fix)
 *
 * Fixes the SyntaxError: `import` can only be used in `import()` or `import.meta` by
 * removing unsafe references like `typeof import !== 'undefined'`. We now safely
 * read env vars via `import.meta.env` inside a try/catch, with a window fallback.
 *
 * Other notes:
 * - Clipboard/export actions remain user-initiated per browser policy.
 * - No static CDN import of Supabase; dynamic import only if env vars present.
 * - Includes lightweight self-tests (console.assert) and adds a couple more.
 */

// ────────────────────────────── Types via JSDoc ─────────────────────────────
/** @typedef {"Amber"|"Amethyst"|"Emerald"|"Ruby"|"Sapphire"|"Steel"} Ink */

/** @typedef {{id:string,name:string,version?:string|null,image_uris?:{digital?:{small?:string,normal?:string,large?:string}},cost?:number|null,inkwell?:boolean,ink?:Ink|null,type?:string[],classifications?:string[]|null,text?:string|null,rarity?:string|null,set?:{id:string,code:string,name:string},collector_number?:string}} Card */
...
const COST_CHOICES = [1,2,3,4,5,6,7,8,9]; // 9 means 9+
...
export default function LorcanaDeckBuilderApp(){
  ...
  // Search
  useEffect(()=>{ (async()=>{ try{ setLoadingCards(true); setErr(null); const res=await searchCards(query||'', uniqueMode); setCards(res); } catch(e){ setErr(e?.message||String(e)); } finally{ setLoadingCards(false); } })(); }, [query, uniqueMode]);
              ...
              <div className="flex items-center gap-2">
                <button className={`px-3 py-1.5 rounded-lg bg-emerald-500/20 border border-emerald-400/40 text-emerald-200 text-sm ${publishing?'opacity-60 cursor-wait':''}`} disabled={publishing} onClick={async()=>{
                  ...
                  try {
                    ...
                    // Safely read env vars (Vite) without tripping the parser
                    ...
                    if (!supaUrl && typeof window !== 'undefined') {
                      // Optional window-provided fallback for local dev
                      supaUrl = window.VITE_SUPABASE_URL; // may be undefined
                      supaKey = window.VITE_SUPABASE_ANON_KEY; // may be undefined
                    }
...
function runSelfTests(){
  console.assert(buildQuery({ text:'n:Stitch', inks:[], types:[], rarities:[], sets:[], inkwell:'any', costs:[], keywords:[], archetypes:[], format:'any' }).includes('name:'), 'buildQuery name prefix');
  console.assert(buildQuery({ text:'e:draw', inks:[], types:[], rarities:[], sets:[], inkwell:'any', costs:[], keywords:[], archetypes:[], format:'any' }).includes('text:'), 'buildQuery text prefix');
  const mock = { a:{card:{id:'a',name:'A',cost:1,inkwell:true},count:2}, b:{card:{id:'b',name:'B',cost:9,inkwell:false},count:3}, c:{card:{id:'c',name:'C',cost:11,inkwell:true},count:1} };
  const curve = curveDataFromDeck(mock);
  console.assert(curve.find(x=>x.cost==='1')?.count===2 && curve.find(x=>x.cost==='9+')?.count===4, 'curve bins (1, 9+)');
  const inkstats = deckInkableStats(mock);
  console.assert(inkstats.inkable===3 && inkstats.uninkable===3, 'inkable/uninkable sums');
  // Cost button query: 2 and 9+ selected
  const q = buildQuery({ text:'', inks:[], types:[], rarities:[], sets:[], costMin:undefined, costMax:undefined, costs:[2,9], inkwell:'any', keywords:[], archetypes:[], format:'any' });
  console.assert(q.includes('c=2') && q.includes('c>=9'), 'cost buttons encoded');
  // Format filter
  console.assert(buildQuery({ text:'', inks:[], types:[], rarities:[], sets:[], costMin:undefined, costMax:undefined, costs:[], inkwell:'any', keywords:[], archetypes:[], format:'core' }).includes('format:core'), 'core format encoded');
  console.assert(buildQuery({ text:'', inks:[], types:[], rarities:[], sets:[], costMin:undefined, costMax:undefined, costs:[], inkwell:'non-inkable', keywords:[], archetypes:[], format:'any' }).includes('-iw'), 'non-inkable encoded');
}
